function distributeGifts(packOfGifts: string[], reindeers: string[]) {
  // Pesos de los regalos (regalosPeso)
  let regalosPeso: number = 0;
  for (let regalo of packOfGifts) regalosPeso += regalo.length;
  // Pesos de los renos (maxRenosPeso)
  let maxRenosPeso: number = 0;
  for (let reno of reindeers) maxRenosPeso += 2 * reno.length;
  return Math.floor(maxRenosPeso / regalosPeso);
}
